        ...
        {team.map((member) => (
          <div key={member.id} className="bg-white shadow-md rounded-2xl p-6 flex gap-4 items-start">
            ...
            <div>
              ...
              {member.role && <p className="text-sm text-[#6B1F2A] mb-1">{member.role}</p>}
              {member.description && <p className="text-sm text-[#231F1E]/60">{member.description}</p>}
            </div>
          </div>
        ))}
        {team.length === 0 && (
          <p className="text-sm text-[#231F1E]/50">No team members added yet.</p>
        )}
      </div>
