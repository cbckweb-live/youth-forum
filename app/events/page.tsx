  ...
  return (
    <main className="px-4 sm:px-8 py-12 sm:py-16 max-w-3xl mx-auto dark:text-[#e5e5e5]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8">
        ...
        <Link href="/events/archive" className="text-sm text-[#6B1F2A] dark:text-[#B84C5C] hover:underline">
          View Past Years →
        </Link>
      </div>
      ...
      {events && events.length === 0 && (
        <p className="text-[#231F1E]/60 dark:text-gray-400">No events added for {currentYear} yet.</p>
      )}

      {upcoming.length > 0 && (
        <div className="mb-10">
          <p className="text-xs uppercase tracking-widest text-[#6B1F2A] dark:text-[#B84C5C] mb-4">Upcoming</p>
      ...
      {past.length > 0 && (
        <div id="past-events">
          <p className="text-xs uppercase tracking-widest text-[#231F1E]/40 dark:text-gray-500 mb-4">Past Events</p>
          <div className="space-y-4">
            {past.map((event) => (
              <EventCard key={event.id} title={event.title} event_date={event.event_date} event_end_date={event.event_end_date} description={event.description} image_url={event.image_url} />
            ))}
